...
function teacher_() {
  return "<!DOCTYPE html><html lang='ko'><head><meta charset='utf-8'>" +
    ...
    /* 1단계 — 프롬프트 만들기 */
    "<div class='card'><h2>1단계 · 문항 만들기</h2>" +
    "<div class='r r3'>" +
    "<div><label>학년</label><select id='pg'><option>1</option><option selected>2</option><option>3</option></select></div>" +
    "<div><label>제한시간(분)</label><select id='pt'><option>5</option><option selected>10</option><option>15</option><option>20</option><option value='0'>무제한</option></select></div>" +
    "<div><label>영역 (선택)</label><select id='pa'><option value='auto'>자동으로 고르기</option></select></div></div>" +
    "<div class='r'><div><label>수업 주제 <span style='color:var(--ro)'>*</span></label>" +
    "<input id='pq' placeholder='예) 광합성에 필요한 물질과 산물 — 검정말 BTB 실험'></div></div>" +
    "<div class='r'><div><label>추가 요청 (선택)</label>" +
    "<input id='pn' placeholder='예) 오늘 한 실험 상황을 2문항에 넣어주세요'></div></div>" +
    "<div class='br'>" +
    "<button class='btn g' id='mk'>프롬프트 만들고 복사</button>" +
    "<a class='btn v' id='gem' href='https://gemini.google.com/app' target='_blank' rel='noopener' style='text-decoration:none'>제미나이 열기</a>" +
    "</div>" +
    "<div id='pbox' class='hd' style='margin-top:13px'>" +
    "<textarea id='ptext' readonly style='min-height:120px'></textarea>" +
    "<p class='hint' style='margin-top:8px'>복사되었습니다. <b>제미나이 열기</b> → 붙여넣기 → 나온 코드를 아래 2단계에 붙여넣으세요.</p></div></div>" +

    /* 2단계 — 등록 */
    "<div class='card'><h2>2단계 · 문항 등록</h2>" +
    "<p class='hint' style='margin-bottom:12px'>AI가 준 <b>문항 코드</b>를 그대로 붙여넣으세요. 앞뒤에 인사말이 붙어 있어도 괜찮습니다.</p>" +
    ...
    "function openMain(){hide('lock');show('main');load();fillAreas()}" +
    ...
    /* 1단계 — 프롬프트 생성 */
    "function fillAreas(){run('areaList',[],function(l){" +
    " $('pa').innerHTML='<option value=\"auto\">자동으로 고르기</option>'+l.map(function(a){" +
    "  return '<option value=\"'+a.i+'\">'+esc(a.name)+' ('+a.n+')</option>'}).join('')})}" +

    "$('mk').onclick=function(){hide('ecard');" +
    " var t=$('pq').value.trim();" +
    " if(!t){toast('수업 주제를 입력하세요.',true);$('pq').focus();return}" +
    " run('buildPrompt',[PIN,{grade:$('pg').value,topic:t,note:$('pn').value.trim()," +
    "  time:Number($('pt').value),area:$('pa').value}],function(p){" +
    "   show('pbox');$('ptext').value=p;copyText(p);" +
    "   window.scrollTo({top:$('pbox').offsetTop-90,behavior:'smooth'})},function(e){showErr(e,'프롬프트 만들기')})};" +
...
const STD_REF = [{"a":"과학과 인류의 지속가능한 삶","s":[["9과01-01","과학적 탐구 방법을 이해하고, 일상생활의 문제에 대한 과학적 해결 방안을 제안할 수 있다"],["9과01-02","과학의 발전이 인류 문명에 미친 영향을 이해하고, 인공지능 등 첨단 과학기술이 가져올 미래 사회의 변화를 조사하여 발표할 수 있다"],["9과01-03","인류의 지속가능한 삶을 위한 과학기술의 중요성과 역할에 대해 토의하고, 개인과 사회 차원의 활동 방안을 찾아 실천할 수 있다"]]},{"a":"생물의 구성과 다양성","s":[["9과02-01","세포는 생명 활동이 일어나는 기본 단위임을 이해하고, 세포의 구조와 기능의 관계를 추론할 수 있다"],["9과02-02","생물의 유기적 구성 단계를 이해하고, 동물과 식물을 비교하여 분석할 수 있다"],["9과02-03","생물다양성을 이해하고, 변이와 생물다양성의 관계를 추론할 수 있다"],["9과02-04","종의 개념과 분류 체계를 이해하고, 생물을 계 수준에서 분류할 수 있다"],["9과02-05","생물다양성 보전의 필요성을 이해하고, 생물다양성 유지를 위한 방안을 조사하고 실천할 수 있다"]]},{"a":"열","s":[["9과03-01","온도와 열평형 과정을 물질을 구성하는 입자들의 배치나 움직임 등으로 설명할 수 있다"],["9과03-02","열은 전도, 대류, 복사로 전달됨을 알고, 열전달 과정을 모형 등을 사용하여 다양하게 표현할 수 있다"],["9과03-03","물질에 따라 비열과 열팽창 정도가 다름을 알고, 이러한 성질이 일상생활에서 유용하게 활용됨을 인식할 수 있다"]]},{"a":"물질의 상태 변화","s":[["9과04-01","확산 및 증발 현상을 관찰하여 물질을 구성하는 입자가 운동하고 있음을 추론할 수 있다"],["9과04-02","물질의 세 가지 상태의 특징을 설명하고, 이를 입자 모형으로 표현할 수 있다"],["9과04-03","여러 가지 물질의 상태 변화를 관찰하고, 이를 입자 모형으로 설명할 수 있다"],["9과04-04","물질의 상태 변화와 열에너지 출입 관계를 이해하고, 이를 실생활에 적용하여 과학의 유용성을 인식할 수 있다"]]},{"a":"힘의 작용","s":[["9과05-01","물체에 작용하는 힘을 화살표를 이용하여 나타내고, 힘의 평형을 이루는 조건을 설명할 수 있다"],["9과05-02","중력, 탄성력, 마찰력, 부력을 이해하고, 각 힘의 특징을 크기와 방향으로 설명할 수 있다"],["9과05-03","알짜힘이 0이 아닐 때 물체의 운동 상태가 변함을 알고, 그 예를 조사하여 분류할 수 있다"],["9과05-04","다양한 사례에서 작용하는 힘과 힘의 평형 관계를 설명하고, 일상생활에서 힘의 특징을 이용한 기구나 장치를 설계할 수 있다"]]},{"a":"기체의 성질","s":[["9과06-01","압력의 의미를 알고, 기체의 압력을 입자의 운동으로 설명할 수 있다"],["9과06-02","기체의 압력과 부피 관계를 실험 결과로부터 알아내고, 이를 입자 모형으로 해석할 수 있다"],["9과06-03","기체의 온도와 부피 관계를 실험 결과로부터 알아내고, 이를 입자 모형으로 해석할 수 있다"]]},{"a":"태양계","s":[["9과07-01","태양계를 구성하는 천체의 특징을 알고, 행성을 목성형 행성과 지구형 행성으로 구분할 수 있다"],["9과07-02","태양의 표면과 대기에서 일어나는 현상을 알고, 태양의 활동이 지구에 미치는 영향을 추론할 수 있다"],["9과07-03","지구 자전에 의한 천체의겉보기 운동과 지구 공전에 의한 별자리 변화를 이해하고, 밤하늘 천체에 호기심을 가진다"],["9과07-04","달을 관측하여 달의 위상변화 원리를 이해하고, 일식과 월식을 설명할 수 있다"]]},{"a":"물질의 특성","s":[["9과08-01","물질의 특성의 의미를 알고, 실험을 통해 밀도, 용해도, 녹는점, 끓는점 등을 설명할 수 있다"],["9과08-02","물질의 특성을 근거로 우리 주변의 물질을 순물질과 혼합물로 분류할 수 있다"],["9과08-03","물질의 특성을 이용하여 혼합물이 분리되는 원리를 이해하고, 이를 이용한 사례를 주변에서 찾을 수 있다"]]},{"a":"지권의 변화","s":[["9과09-01","지구계의 구성 요소를 알고, 지권의 층상 구조와 그 특징을 조사·발표할 수 있다"],["9과09-02","조암 광물의 주요 특성을 관찰하고, 암석과 광물의 활용 방안 및 자원으로서 가치에 대해 조사할 수 있다"],["9과09-03","지각을 이루는 암석을 생성 과정에 따라 분류하고, 암석의 순환 과정을 설명할 수 있다"],["9과09-04","풍화 과정을 이해하고, 토양 생성 과정을 풍화 작용의 예로 설명할 수 있다"],["9과09-05","대륙이동설을 이해하고, 지진과 화산이 발생하는 지역의 분포를 판의 경계와 관련지어 설명할 수 있다"]]},{"a":"빛과 파동","s":[["9과10-01","빛의 반사와 굴절의 원리를 이해하고, 물체를 보는 과정을 빛의 경로를 이용하여 표현할 수 있다"],["9과10-02","평면거울에서 상이 생기는 원리를 설명하고, 일상생활에서 사용되는 거울과 렌즈의 종류를 분류하고 상의 특징을 비교할 수 있다"],["9과10-03","물체의 색을 빛의 반사와 관련지어 설명하고, 영상 장치에서 빛의 합성을 이용하여 다양한 색이 표현되는 원리를 이해할 수 있다"],["9과10-04","파동의 발생과 전달 과정을 이해하고, 소리의 특성을 진폭, 진동수, 파형 등의 과학적 용어로 표현할 수 있다"]]},{"a":"물질의 구성","s":[["9과11-01","원소와 화합물의 정의를 알고, 원소와 화합물을 화학식으로 표현할 수 있다"],["9과11-02","원소를 구성하는 입자인 원자는 양성자, 중성자, 전자로 구성되며, 양성자의 수에 따라 원소의 종류가 달라짐을 입자 모형을 활용하여 설명할 수 있다"],["9과11-03","원소는 양성자의 수에 따라 주기율표에 배치됨을 알고, 주기율표에서 성질이 유사한 원소를 찾을 수 있다"],["9과11-04","물질을 이루는 입자는 원자, 분자, 이온 등으로 존재할 수있음을 알고, 이온은 전하를 띠고 있음을 설명할 수 있다"]]},{"a":"식물과 에너지","s":[["9과12-01","광합성 과정을 이해하고, 환경 요인과 광합성의 관계를 탐구하는 실험을 설계할 수 있다"],["9과12-02","식물의 호흡과 광합성의 관계를 이해하고, 호흡과 광합성 과정에서 출입하는 에너지와 물질의 변화를 분석할 수 있다"],["9과12-03","광합성 산물의 저장과 이용 과정을 이해하고, 모형으로 설명할 수 있다"]]},{"a":"동물과 에너지","s":[["9과13-01","소화계의 구조와 기능을 이해하고, 소화 과정을 소화 효소의 작용과 관련지어 추론할 수 있다"],["9과13-02","순환계의 구조와 기능을 이해하고, 혈액의 순환 경로를 종합하여 발표할 수 있다"],["9과13-03","호흡계의 구조와 기능을 이해하고, 호흡 운동의 원리를 나타내는 모형을 만들 수 있다"],["9과13-04","배설계의 구조와 기능을 이해하고, 노폐물이 배설되는 과정을 모식도로 표현할 수 있다"],["9과13-05","동물이 세포호흡을 통해 에너지를 얻는 과정을 소화, 순환, 호흡, 배설과 관련지어 통합적으로 설명할 수 있다"]]},{"a":"전기와 자기","s":[["9과14-01","마찰 전기, 정전기 유도 현상을 관찰하고, 이를 전기력과 원자 모형을 이용하여 설명할 수 있다"],["9과14-02","전기 회로에서 전류를 모형으로 설명하고, 실험을 통해 저항, 전류, 전압 사이의 관계를 이끌어낼 수 있다"],["9과14-03","저항의 직렬연결과 병렬연결의 특징을 비교하고, 일상생활에서 전기 에너지가 다양한 형태의 에너지로 전환됨을 소비 전력과 관련지어 설명할 수 있다"],["9과14-04","자기장 안에 놓인 전류가 흐르는 코일이 받는 힘의 특성을 추리하고, 이를 활용한 예를 조사하여 설명할 수 있다"]]},{"a":"별과 우주","s":[["9과15-01","연주시차를 이용하여 별까지의 거리를 구할 수 있고, 별의 등급과 밝기의 관계 및 표면 온도와 색의 관계를 설명할 수 있다"],["9과15-02","우리은하의 구조와 크기를 이해하고, 성운과 성단의 특징을 비교할 수 있다"],["9과15-03","모형을 이용하여 우주가 팽창하고 있음을 설명할 수 있다"],["9과15-04","우주탐사의 의의와 인류에게 미치는 영향을 조사하여 과학의 유용성을 인식할 수 있다"]]},{"a":"화학 반응의 규칙성","s":[["9과16-01","물리 변화와 화학 변화의 의미를 알고, 화학 변화에서 새로운 물질이 생성됨을 관찰할 수 있다"],["9과16-02","간단한 화학 반응을 화학 반응식으로 표현하고, 화학 반응식에서 계수의 비를 입자 수의 비로 해석할 수 있다"],["9과16-03","화학 반응에서 질량이 보존됨을 실험을 통해 추론할 수 있다"],["9과16-04","화합물을 구성하는 성분 원소의 질량비가 일정함을 실험 자료를 해석하여 설명할 수 있다"],["9과16-05","기체 반응에서 기체 부피 사이의 비가 일정함을 실험 자료를 해석하여 설명할 수 있다"],["9과16-06","화학 반응에서 열에너지가 출입함을 알고, 생활 속 사례를 조사하여 발표할 수 있다"]]},{"a":"날씨와 기후변화","s":[["9과17-01","지구 대기권을 4개 권역으로 구분하며, 온실효과와 지구온난화를 복사 평형의 관점으로 설명할 수 있다"],["9과17-02","대기 대순환에서 위도별 바람의 특성을 파악하고, 대기 대순환의 역할을 설명할 수 있다"],["9과17-03","상대습도, 단열 팽창 및 응결 현상의 관계를 이해하고, 구름의 생성과 강수 과정을 설명할 수 있다"],["9과17-04","기압, 기단, 전선의 개념을 이해하고, 일기도에서 저기압과 고기압의 분포에 따른 날씨를 해석할 수 있다"]]},{"a":"수권과 해수의 순환","s":[["9과18-01","수권에서 해수, 담수, 빙하의 분포와 활용 사례를 조사하고, 자원으로서 물의 가치에 대해 토론할 수 있다"],["9과18-02","해수의 수온과 염분의 분포 및 변화를 해석하여 해수의 특성을 설명할 수 있다"],["9과18-03","대기 대순환과 해양 표층 순환과의 관계를 이해하고, 기후변화에 영향을 미치는 해류의 역할을 설명할 수 있다"]]},{"a":"운동과 에너지","s":[["9과19-01","직선상에서 움직이는 물체의 운동을 그래프로 나타내고 해석할 수 있다"],["9과19-02","자유 낙하하는 물체의 운동에서 시간에 따른 속력의 변화가 일정함을 분석할 수 있다"],["9과19-03","일의 정의를 알고, 자유 낙하하는 물체의 운동에서 중력이 한 일을 위치 에너지와 운동 에너지로 표현할 수 있다"],["9과19-04","물체의 운동에서 역학적 에너지의 전환과 보존을 이해하고, 이를 활용하여 일상생활 속 물체의 운동을 예측할 수 있다"]]},{"a":"자극과 반응","s":[["9과20-01","감각기관의 구조와 기능을 이해하고, 실험을 통해 자극이 뇌로 전달되는 과정을 추론할 수 있다"],["9과20-02","뉴런과 신경계의 구조와 기능을 이해하고, 자극에서 반응이 일어나기까지의 과정을 모형으로 설명할 수 있다"],["9과20-03","우리 몸의 기능 조절에 호르몬이 관여함을 알고, 관련 자료를 조사하여 발표할 수 있다"]]},{"a":"생식과 유전","s":[["9과21-01","개체의 생장에 세포분열이 필요한 이유를 세포의 표면적과 부피의 관계로 추론할 수 있다"],["9과21-02","염색체와 유전자의 관계를 이해하고, 체세포분열과 생식세포 형성과정의 특징을 염색체 행동을 중심으로 해석할 수 있다"],["9과21-03","수정란으로부터 개체가 발생하는 과정을 모형으로 표현할 수 있다"],["9과21-04","멘델 유전 실험의 의의와 원리를 이해하고, 멘델 유전 원리가 적용되는 유전 현상을 조사하여 협력적으로 소통할 수 있다"],["9과21-05","사람의 유전 형질과 유전 연구 방법을 알고, 가계도를 분석하여 사람의 유전 현상을 설명할 수 있다"]]},{"a":"재해·재난과 안전","s":[["9과22-01","재해·재난 사례와 관련된 자료를 조사하고, 그 발생 원인과 피해에 대해 과학적으로 분석할 수 있다"],["9과22-02","과학적 원리를 이용하여 재해 ·재난에 대한 대비 및 대처 방안을 세울 수 있다"]]},{"a":"과학과 나의 미래","s":[["9과23-01","과학과 관련된 직업의 종류와 하는 일을 조사하고, 과학기술의 발달로 생기는 미래 사회의 직업 변화를 예상할 수 있다"],["9과23-02","자신의 진로와 관련 있는 과학 분야를 조사하고, 진로 선택을 위하여 필요한 과학 학습을 계획할 수 있다"]]}];
function areaList() {
  return STD_REF.map(function (x, i) { return { i: i, name: x.a, n: x.s.length }; });
}

const PROMPT_HEAD = [
  '당신은 대한민국 중학교 과학 교사이자 평가 문항 개발 전문가입니다.',
  '2022 개정 교육과정 중학교 과학 성취기준에 근거하여, 수업 직후 5~10분 안에 푸는',
  '형성평가 10문항을 만들고, 아래 JSON 형식으로만 출력하세요.',
  '',
  '■ 수업 정보'
].join('\n');
...
const PROMPT_FORM = [
  '',
  '■ 출력 형식 (정확히 지킬 것)',
  'JSON 하나만 출력합니다. 설명 문장을 앞뒤에 붙이지 마세요.',
  '',
  '{',
  ' "title": "수업 주제를 20자 이내로",',
  ' "grade": 2,',
  ' "code": "9과12-01",',
  ' "time": 10,',
  ' "items": [',
  '  {"t":"MC","lv":"하","q":"질문","c":["선택1","선택2","선택3","선택4","선택5"],"a":"3","e":"해설"},',
  '  {"t":"SA","lv":"상","q":"질문","a":"정답","ok":["정답","다른 표기"],"e":"해설"}',
  ' ]',
  '}',
  '',
  '항목 뜻',
  '  t   "MC" = 5지선다 객관식, "SA" = 단답형',
  '  lv  난이도 "하" / "중" / "상"',
  '  q   질문 문장',
  '  c   MC일 때만. 선택지 정확히 5개. SA면 아예 쓰지 마세요.',
  '  a   MC는 정답 선택지 번호를 문자열로 ("1"~"5"). SA는 정답 낱말.',
  '  ok  SA일 때만. 인정할 표기 3~6개. 반드시 a 값도 포함.',
  '  e   해설. 학생이 제출 직후 읽습니다.',
  '',
  '■ 반드시 지킬 규칙',
  '1. 총 10문항. 1~8번 MC, 9~10번 SA.',
  '2. 난이도 배분: 하 3, 중 5, 상 2. 앞 번호일수록 쉽게.',
  '3. MC 정답 번호를 한쪽에 몰지 말 것. 1~5번에 고르게 (예: 3,5,1,4,2,1,5,3).',
  '4. SA 정답은 15자 이내 핵심 용어. ok에 띄어쓰기 차이·동의어·기호 표기(산소/O2)를 넣을 것.',
  '5. 계산 문항 최대 1개, 암산으로 풀리는 수치만.',
  '6. 그림·표·그래프가 있어야만 풀리는 문항 금지. 필요한 정보는 문장에 다 적을 것.',
  '7. "옳지 않은 것은?" 유형 최대 2문항.',
  '8. 선택지 길이를 비슷하게. 정답만 길면 티가 납니다.',
  '9. 교과서 용어 사용. 고등학교 수준 개념 금지.',
  '10. 해설은 1~2문장. 왜 그 답인지 + 흔한 오개념 하나.',
  '11. 모든 문장 한국어. 질문은 2문장 이내.',
  '12. 출력은 JSON 하나뿐. 인사말·설명·마크다운 제목 금지.',
  ''
].join('\n');

/** 제미나이 등에 붙여넣을 프롬프트를 완성해 준다 */
function buildPrompt(pin, cfg) {
  checkPin_(pin);
  cfg = cfg || {};
  const grade = String(cfg.grade || '').trim();
  const topic = String(cfg.topic || '').trim();
  const note = String(cfg.note || '').trim();
  const time = Number(cfg.time) || 10;
  const ai = String(cfg.area);

  if (!topic) throw new Error('수업 주제를 입력하세요.');

  let picked;
  if (ai !== '' && ai !== 'auto' && STD_REF[Number(ai)]) {
    picked = [STD_REF[Number(ai)]];
  } else {
    picked = STD_REF;
  }

  const ref = picked.map(function (x) {
    return '【' + x.a + '】\n' + x.s.map(function (p) { return '  ' + p[0] + ' ' + p[1]; }).join('\n');
  }).join('\n');

  const info = [
    '학년: 중학교 ' + (grade || '2') + '학년',
    '수업 주제: ' + topic,
    note ? '추가 요청: ' + note : '추가 요청: 없음',
    '제한시간: ' + time + '분',
    '',
    picked.length === 1
      ? '아래 성취기준 중 수업 주제에 가장 알맞은 것 하나를 골라 code 에 쓰세요.'
      : '아래 87개 성취기준 중 수업 주제에 가장 알맞은 것 하나를 골라 code 에 쓰세요.'
  ].join('\n');

  return PROMPT_HEAD + '\n' + info + '\n' + PROMPT_FORM +
    '\n■ 2022 개정 중학교 과학 성취기준' +
    (picked.length === 1 ? ' (' + picked[0].a + ' 영역)' : ' (87개)') + '\n' +
    ref + '\n\n이제 JSON을 출력하세요.\n';
}
